...
namespace GetBomMaterialMessageTool.Task
{
    public class ImportDb
    {
        ...
        /// <returns></returns>
        public DataTable ImportExcelToDt(DataTable materialdt,string fileAddress)
        {
            ...
            try
            {
                ...
                //将从EXCEL过来的记录集为空的行清除
                importdt = RemoveEmptyRows(importExcelDt);
                //将importdt与materialdt进行合并,获取其对应的fmaterialid,并最后返回至dt
                dt = Merge(materialdt,importdt);
            }
            catch (Exception)
            {
                dt.Rows.Clear();
